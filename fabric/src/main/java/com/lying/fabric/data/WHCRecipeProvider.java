...
public class WHCRecipeProvider extends FabricRecipeProvider
{
	...
	{
		ComplexRecipeJsonBuilder.create(WHCSpecialRecipes.CANE_SWORD_SERIALIZER.get()).offerTo(exporter, "cane_sword");
		...
		offerMetalWheelRecipe(exporter, WHCItems.WHEEL_COPPER.get(), Metal.COPPER);
		offerMetalWheelRecipe(exporter, WHCItems.WHEEL_IRON.get(), Metal.IRON);
		offerMetalWheelRecipe(exporter, WHCItems.WHEEL_GOLD.get(), Metal.GOLD);
		offerMetalWheelRecipe(exporter, WHCItems.WHEEL_NETHERITE.get(), Metal.NETHERITE);
		
		offerHandleRecipe(exporter, WHCItems.HANDLE_BONE.get(), Ingredient.ofItems(Items.BONE), "bone_handle");
		offerHandleRecipe(exporter, WHCItems.HANDLE_COPPER.get(), Ingredient.ofItems(Items.COPPER_INGOT), "copper_handle");
		offerHandleRecipe(exporter, WHCItems.HANDLE_IRON.get(), Ingredient.ofItems(Items.IRON_INGOT), "iron_handle");
		offerHandleRecipe(exporter, WHCItems.HANDLE_GOLD.get(), Ingredient.ofItems(Items.GOLD_INGOT), "gold_handle");
		offerHandleRecipe(exporter, WHCItems.HANDLE_SKULL.get(), Ingredient.ofItems(Items.SKELETON_SKULL), "skull_handle");
		offerHandleRecipe(exporter, WHCItems.HANDLE_WITHER.get(), Ingredient.ofItems(Items.WITHER_SKELETON_SKULL), "wither_handle");
		...
		ShapedRecipeJsonBuilder.create(RecipeCategory.MISC, WHCItems.STOOL.get())
	...
	private static void offerMetalWheelRecipe(Consumer<RecipeJsonProvider> exporter, Item wheel, Metal metal)
	{
	...
	private static void offerWheelchairRecipe(Consumer<RecipeJsonProvider> exporter, Item chair, Wood wood)
	{
	...
	private static void offerHandleRecipe(Consumer<RecipeJsonProvider> exporter, Item handle, Ingredient material, String name)
	{
		RecipeHandleJsonBuilder builder = new RecipeHandleJsonBuilder(handle.getDefaultStack(), material, RecipeCategory.TRANSPORTATION);
		if(material.getMatchingStacks().length > 0)
			builder.criterion("has_backing", RecipeProvider.conditionsFromItem(material.getMatchingStacks()[0].getItem()));
