...
public class ChairInventoryScreen extends HandledScreen<ChairInventoryScreenHandler>
{
	private static final MinecraftClient mc = MinecraftClient.getInstance();
	public static final Identifier TEXTURE = Reference.ModInfo.prefix("textures/gui/chair_inventory.png");
	...
	public void handledScreenTick()
	{
		...
		unbindButton.visible = chair.hasUpgrade(WHCChairUpgrades.HANDLES.get());
	...
	protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY)
	{
		...
		context.drawTexture(RenderLayer::getGuiTextured, TEXTURE, i, j, 0, 0, 174, 164, 256, 256);
		
		if(getScreenHandler().hasStorage)
			context.drawTexture(RenderLayer::getGuiTextured, TEXTURE, this.width / 2 - 45, this.height / 2 - 66, 0, 164, 90, 54, 256, 256);
		
		if(getScreenHandler().hasPlacer)
			context.drawTexture(RenderLayer::getGuiTextured, TEXTURE, this.width / 2 + 54, this.height / 2 - 48, 0, 218, 18, 18, 256, 256);
	}
