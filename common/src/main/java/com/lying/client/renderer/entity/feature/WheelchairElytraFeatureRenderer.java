...
public class WheelchairElytraFeatureRenderer<T extends EntityWheelchair> extends EntityFeatureRenderer<T>
{
	...
	{
		if(entity.getFirstPassenger() != null && entity.getFirstPassenger() instanceof AbstractClientPlayerEntity)
		{
				AbstractClientPlayerEntity playerEntity = (AbstractClientPlayerEntity)entity.getFirstPassenger();
				if(playerEntity.getElytraTexture() != null)
					return playerEntity.getElytraTexture();
				else if(playerEntity.getCapeTexture() != null && playerEntity.isPartVisible(PlayerModelPart.CAPE))
					return playerEntity.getCapeTexture();
				else
					return TEXTURE;
		}
		return TEXTURE;
